...
foreign lib {
	...
	LoadSound :: proc(fileName: cstring, looping := false) -> Sound --- // Load sound from file
	LoadSoundFromWave :: proc(wave: Wave, looping := false) -> Sound --- // Load sound from wave data
	LoadSoundAlias :: proc(source: Sound, looping := false) -> Sound --- // Create a new sound that shares the same sample data as the source sound, does not own the sound data
